...
def compare_docs(vcts):
    ...
    if v_sim >= 0.7:
        messagebox.showwarning(
            f"Warning", f"Plagiarism Detected!!!\n{(v_sim * 100)} % Precision"
        )
        ...
        messagebox.showwarning(
            "Warning", f"No plagiarism detected!\n{(v_sim * 100)}% Precision"
        )
...
boton_central = tk.Button(
    frame_principal, text="Verify", command=lambda: compare_docs(vcts)
)
...
boton_derecha = tk.Button(
    frame_derecha, text="Load File", command=lambda: v.cargar_archivo(texto_derecha, "")
)
...
boton_cerrar = tk.Button(
    frame_principal, text="Cerrar Ventana", command=ventana.destroy
)
boton_cerrar.pack()
